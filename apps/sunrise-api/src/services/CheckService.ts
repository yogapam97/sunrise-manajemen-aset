...
export default class CheckService extends ApiService {
  public static checkValidationSchema: Schema = {
    fixed_asset: { exists: true, errorMessage: "Fixed Asset is required" },
    assignee: {
      custom: {
        options: (value, { req }) => {
          if (!value && !req.body.location) {
            throw new Error("Either Assignee or Location is required");
          }
          if (value) {
            return MemberService.checkInputMemberExist(value, { req });
          }
          return true;
        },
      },
    ...
    location: {
      custom: {
        options: (value, { req }) => {
          if (!value && !req.body.assignee) {
            throw new Error("Either Assignee or Location is required");
          }
          if (value) {
            return LocationService.checkInputLocationExist(value, { req });
          }
          return true;
        },
      },
  ...
  public static async getAll(
    ...
    const checks = await Check.paginate(query, {
      populate: [
        {
          path: "fixed_asset",
    ...
    return {
      checks: checks.docs,
      pagination: {
        total: checks.totalDocs,
        limit: checks.limit,
        page: checks.page,
        totalPages: checks.totalPages,
        pagingCounter: checks.pagingCounter,
        hasPrevPage: checks.hasPrevPage,
        hasNextPage: checks.hasNextPage,
        prevPage: checks.prevPage,
        nextPage: checks.nextPage,
      },
    };
  }

  public static async getById(checkId: string): Promise<any> {
    await CheckService.checkIdValid(checkId, "Check");
  ...
  public static async createCheck(checkModel: ICheck): Promise<any> {
    const createdCheck = await Check.create(checkModel);
    await FixedAssetService.updateFixedAsset(checkModel.fixed_asset, {
      current_check: createdCheck,
    });
    return createdCheck;
  }
  ...
  private static formatData(data: any) {
    return data.map((item: any) => ({
      ...
      status: item.status === "check-in" ? "Check In" : "Check Out",
      ...
      note: item.note,
      checked_by: `Name: ${item?.checked_by?.user?.name}|Email: ${item?.checked_by?.email}|Code: ${item?.checked_by?.code}`,
    }));
  }

  public static async downloadCheck(
    workspace: string,
    options: {
      search: string;
      status: any;
    ...
    const formattedData = this.formatData(checks);
