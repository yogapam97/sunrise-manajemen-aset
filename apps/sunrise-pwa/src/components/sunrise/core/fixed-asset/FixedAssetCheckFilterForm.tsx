...
type FixedAssetCheckFilterFormProps = {
  defaultFilter?: any;
  onFilter: (values: any) => void;
  config?: any;
};

const FixedAssetCheckFilterSchema: Yup.ObjectSchema<any> = Yup.object().shape({
...
export default function FixedAssetCheckFilterForm({
  defaultFilter,
  onFilter,
  config,
}: FixedAssetCheckFilterFormProps) {
  const { workspace } = useWorkspaceContext();

  const defaultValues: any = useMemo(
    () => ({
      status: defaultFilter?.status || ["check-in", "check-out"],
  ...
  const methods = useForm({
    resolver: yupResolver(FixedAssetCheckFilterSchema),
    defaultValues,
  });
  ...
  const handleFixedAssetCheckFilter = (status: string) => {
  ...
  const handleReset = () => {
    reset();
    setValue("status", ["check-in", "check-out"]);
    setValue("assignee", []);
    setValue("location", []);
        ...
        <Stack spacing={1}>
          ...
          <Stack spacing={1} direction="row" alignItems="center">
            <ButtonGroup size="small">
              <Button
                variant={getValues("status").includes("check-in") ? "contained" : "outlined"}
                startIcon={<Iconify icon={CHECK_IN_ICON} />}
                onClick={() => handleFixedAssetCheckFilter("check-in")}
              >
                Check In
              </Button>
              <Button
                variant={getValues("status").includes("check-out") ? "contained" : "outlined"}
                startIcon={<Iconify icon={CHECK_OUT_ICON} />}
                onClick={() => handleFixedAssetCheckFilter("check-out")}
              >
                Check Out
              </Button>
            </ButtonGroup>
