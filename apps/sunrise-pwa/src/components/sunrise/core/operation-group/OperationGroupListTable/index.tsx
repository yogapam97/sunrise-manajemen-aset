...
import OperationGroupTableRow from "./OperationGroupTableRow";
import OperationGroupTableToolbar from "./OperationGroupTableToolbar";
import {
  ALPHA_ASC_ICON,
  ALPHA_DESC_ICON,
  CREATE_NEWEST_ICON,
...
const TABLE_HEAD = [
  ...
  { id: "name", label: "Operation Group", alwaysShow: true },
  { id: "do", label: "Do", show: true },
  { id: "operation", label: "Operation", show: true },
...
type OperationGroupListTableProps = {
  workspaceId: string;
  operationGroups: IOperationGroupItem[];
...
export default function OperationGroupListTable({
  workspaceId,
  operationGroups,
  loading,
  pagination,
    ...
    <Card>
      ...
      <TableContainer sx={{ position: "relative", overflow: "unset" }}>
        <Scrollbar>
          <Table size="medium" sx={{ minWidth: 960 }}>
            <TableHeadCustom headLabel={TABLE_HEAD} rowCount={operationGroups?.length || 0} />
            ...
            <TableBody>
              {loading ? (
                ...
                <>
                  ...
                  {operationGroups.length < 40 && operationGroups.length > 0 && (
                    <TableEmptyRows height={10} emptyRows={50} />
                  )}

                  <TableNoData notFound={!operationGroups.length && !loading} />
                </>
              )}
            </TableBody>
          </Table>
      ...
      {!loading && (
        <TablePaginationCustom
          ...
          rowsPerPageOptions={operationGroups?.length > 50 ? [50, 100, 150, 200, 250] : []}
          dense
        />
      )}
    </Card>
